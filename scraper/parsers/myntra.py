...
class MyntraParser(BaseParser):
    ...
    def _map_api_item(self, item: dict) -> Optional[RawProduct]:
        ...
        image_url = item.get("searchImage") or item.get("image") or _extract_image_url(item)

        rating_val = (
            item.get("rating")
            or item.get("averageRating")
            or (item.get("ratingInfo") or {}).get("rating")
        )
        rating_count = (
            item.get("ratingCount")
            or item.get("totalRatings")
            or (item.get("ratingInfo") or {}).get("ratingCount")
        )
